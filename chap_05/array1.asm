...
segment .data
FirstMsg      db  "First 10 elements for array", 0
Prompt        db  "Enter index of element to display: ", 0
SecondMsg     db  "Element %d is %d", NEW_LINE, 0
ThirdMsg      db  "Element 20 through 29 or array", 0
InputFormat   db  "%d", 0
...
init_loop:
        ...
        call    puts
        pop     ecx                 ; save result as "add esp, 4". But "pop ecx" is single byte instruction, but the "add ..." is three bytes instruction. So the current one save some space for instruction.
